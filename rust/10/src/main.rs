...
impl Rule {
    fn check(&self, reprint: &Reprint) -> bool {
        reprint.pages.is_sorted_by(|l, r| self.ord(l, r) != Ordering::Greater)
    }

    fn fix(&self, reprint: &mut Reprint) {
        reprint.pages.sort_by(|l, r| self.ord(l, r));
    }

    fn ord(&self, l: &i32, r: &i32) -> Ordering {
        if *l == self.before && *r == self.after {
            return Ordering::Less;
        } else if *l == self.after && *r == self.before {
...
fn main() {
    ...
    let middle_sum : i32 = reprints.iter()
        ...
        .map(|e| {
            ...
            loop {
                if rules.iter().all(|r| r.check(&fixed)) {
                    return fixed;
                }
                for rule in &rules {
                    rule.fix(&mut fixed);
        ...
        .filter_map(|e| e.middle_page().map(|v| *v))
        .sum();
