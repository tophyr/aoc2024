...
struct rule {
  ...
  bool apply(std::vector<int>& series) const {
    ...
    if (before_idx && after_idx && *before_idx > *after_idx) {
      int should_be_before = series[*before_idx];
      std::rotate(series.begin() + *after_idx, series.begin() + *before_idx, series.begin() + *before_idx + 1);
      return false;
    } else {
      return true;
    }
...
int main(int argc, char* argv[]) {
  ...
  for (auto& s : series) {
    bool fixed{};
    while (!std::all_of(rules.begin(), rules.end(), [&](auto const& rule) { return rule.apply(s); })) {
      fixed = true;
    }
